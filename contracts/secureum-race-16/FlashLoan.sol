...
contract FlashLoan is IERC3156FlashLender {
    ...
    function flashLoan(
        ...
        IERC20(token).approve(address(receiver), oldAllowance + amount); // @note sus

        require(
            receiver.onFlashLoan(msg.sender, token, amount, fee, data) == CALLBACK_SUCCESS,
            'Callback failed'
        ); // @note will fail param

        uint256 newBal = IERC20(token).balanceOf(address(this));
        if (newBal < oldBal + fee) {
            // @note incorrect shoudl be >
            uint retAmt = oldBal + fee - newBal;
            require(
                IERC20(token).transferFrom(msg.sender, address(this), retAmt), // incorrect shoudl transfer oldBal + fee
